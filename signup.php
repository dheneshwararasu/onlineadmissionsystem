		...
		<script type="text/javascript">
			function validate() {
				$('#signup input[type="text"]').each(function() {
					if (this.required) {
						if (this.value == "") $(this).addClass("inpterr");
						else $(this).addClass("inpterrc");
					}
					if ($(this).attr("VT") == "NM") {
                        if (isset($_SESSION[this.value])) {
                    ...
                    if ($(this).attr("VT") == "PW") {
						if (!IsPassword($(this).val()) && this.value != "") {
            ...
            function IsPassword(x) {
                // Password should contain at least one uppercase letter, one lowercase letter, one digit, one special character, and is at least 8 characters long.
				var pw = new RegExp(/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[!@#$%^&*(),.?":{}|<>])[A-Za-z\d!@#$%^&*(),.?":{}|<>]{8,}$/);
				return pw.test(x);
			}
			/*		
					function IsPin(x) 
